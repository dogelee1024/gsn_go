...
var abiIRelayhub *abi.ABI

// IRelayhubAbi returns the abi of the IRelayhub contract
func IRelayhubAbi() *abi.ABI {
	return abiIRelayhub
}

var binIRelayhub []byte

func init() {
	var err error
	abiIRelayhub, err = abi.NewABI(abiIRelayhubStr)
	if err != nil {
		panic(fmt.Errorf("cannot parse IRelayhub abi: %v", err))
	}
	if len(binIRelayhubStr) != 0 {
		binIRelayhub, err = hex.DecodeString(binIRelayhubStr[2:])
		if err != nil {
			panic(fmt.Errorf("cannot parse IRelayhub bin: %v", err))
		}
	}
}
